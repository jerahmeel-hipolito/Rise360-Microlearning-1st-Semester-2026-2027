...
// ---------- Test 1: deriveLessonMetadata parses a normal lesson path ----------
{
  ...
  check(
    'deriveLessonMetadata parses a standard lesson path',
    meta && meta.courseId === 'biostatistics' && meta.chapterId === '01' && meta.lessonId === '01',
    JSON.stringify(meta)
  );
}
...
// ---------- Test 2: deriveLessonMetadata handles a GitHub Pages project subpath ----------
{
  ...
  const meta = sandbox.Tracking.deriveLessonMetadata();
  check(
    'deriveLessonMetadata handles a GitHub Pages project subpath',
    meta && meta.courseId === 'ssa' && meta.chapterId === '02' && meta.lessonId === '03',
    JSON.stringify(meta)
  );
}
...
// ---------- Test 5: api.js sends text/plain content-type, not application/json ----------
{
  ...
  sandbox.Storage.setStudentId('2025-0001');

  // Run as an async IIFE inside the sandbox to await Api.send
  vm.runInContext(`
    (async () => {
      global.__sendPromise = Api.send({ studentId: '2025-0001', sessionId: 's1', status: 'Completed' });
    })();
  `, sandbox);
}
...
(async () => {
  ...
  const sandbox = freshSandbox('/courses/biostatistics/chapter-01/lesson-01/index.html', fetchSpy);
  await sandbox.Api.send({ studentId: '2025-0001', sessionId: 's1', status: 'Completed' });
  check(
  ...
  // ---------- Test 7: network failure queues the payload instead of losing it ----------
  {
    ...
    await sandbox7.Api.send({ studentId: '2025-0001', sessionId: 's3', status: 'Incomplete' });
  ...
  // ---------- Test 8: a 400 (invalid payload) does NOT get queued for retry ----------
  {
    ...
    await sandbox8.Api.send({ studentId: '2025-0001', sessionId: 's4', status: 'Completed' });
