...
3. **Read engagement on the Dashboard tab**, not the Activity Log directly. Activity Log is meant to stay a raw, append-only table — one row per attempt, never hand-edited. `apps-script/dashboard-formulas.md` has ready-to-paste `QUERY` formulas for per-lesson completion rates, per-student progress, average completion time, and a list of students who haven't started anything yet, all written against this project's real column layout.
